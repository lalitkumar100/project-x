...
export default function TransactionReportPage() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 p-4 pt-0">
      ...
      <SectionHeader
        title="TCG Transaction Report"
        description="View and search transactions from the TradeChainGuardian network."
      />
      ...
      <Card className={`shadow-sm border ${INDIGO.border} flex-1`}>
        ...
        {/* Table header bar */}
        <CardHeader className={`py-3 px-4 border-b ${INDIGO.border} ${INDIGO.header} rounded-t-lg`}>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className={`text-base font-semibold ${INDIGO.accent}`}>
          ...
          {!loading && !error && paged.length > 0 && (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className={`text-xs font-semibold uppercase tracking-wide text-muted-foreground border-b ${INDIGO.border} ${INDIGO.header}`}>
                ...
                <tbody className="divide-y divide-border">
                  {paged.map((row, idx) => (
                    <tr
                      key={row.id}
                      className={`transition-colors ${INDIGO.hoverRow}`}
                    >
                      <td className="px-4 py-3 text-muted-foreground">
                        {(safePage - 1) * PAGE_SIZE + idx + 1}
                      </td>
                      <td className="px-4 py-3 font-mono font-medium text-xs text-indigo-700 dark:text-indigo-300">
                        {row.invoice_id}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          size="icon"
                          variant="ghost"
                          className="h-8 w-8 text-indigo-500 hover:text-indigo-700 hover:bg-indigo-100 dark:hover:bg-indigo-900/30"
                          title="View details"
                          onClick={() => navigate(`/report/transaction/${row.id}`)}
                        >
