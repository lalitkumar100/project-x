 ...
 * reset_db.js
 * ────────────────────────────────────────────────────────────────
 * Connects to the PostgreSQL database defined in back-end/.env,
 * drops ALL existing tables, views, sequences and types in the
 * public schema, then recreates everything from actualSchema.
 *
 * Run:
 *   node back-end/test_config/reset_db.js
 *
 * ⚠️  THIS WILL DELETE ALL DATA — use only in development.
 * ────────────────────────────────────────────────────────────────
 */
...
async function run() {
  ...
  console.log("\n⏳ Dropping all views, tables, sequences …");
  await client.query(`DROP SCHEMA public CASCADE`);
  await client.query(`CREATE SCHEMA public`);
  await client.query(`GRANT ALL ON SCHEMA public TO postgres`);
  await client.query(`GRANT ALL ON SCHEMA public TO public`);
  console.log("🗑️  Public schema wiped clean.");
